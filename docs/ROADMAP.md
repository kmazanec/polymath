...
| **WebSocket message protocol** | `packages/contract/src/wire.ts` (event types + Action wrapping) | F-01 | F-05, F-09, F-11, F-12, F-14, F-17 | Adds new event kinds (e.g., `transfer_submitted`, `explain_back_recording_ended`). Existing event kinds are append-only — never re-shape an existing event's payload. **New optional fields on an existing event are allowed** (they don't break existing senders) — see `submit.repSubmission`, added in I1. | Moderate. Two features extending the wire format in the same iteration must coordinate. **I1 update:** the `submit` event gained an optional `repSubmission` discriminated union (truth_table/circuit/pseudocode), locked in I1 Step 0 before the rep features fanned out — this removed the predicted per-rep submit-union convergence (one shared field instead of three competing edits). The required `submission` string is unchanged. **I1 inner-loop update:** `submit` further gained two optional append-only fields — `correct` (the client's verdict, read only for the agent's tactical move choice; the server recomputes correctness server-side for the integrity-critical BKT/streak) and `responseTimeMs` (for F-09's rule-gate response-time band). The new `transfer_submitted` event kind is live (F-07). **I2 update (F-10):** voice did NOT touch this wire contract — voice flows over WebRTC, not the WebSocket. The voice channel adds (a) one append-only REST route `POST /api/realtime/session` (mints a 5-min room-scoped LiveKit token; rate-limited per session) and (b) a new `events.kind` value `voice_turn` (a string in the existing JSONB event log — no schema migration). **The LiveKit/OpenAI-Realtime network boundary is behind an injectable `RealtimeSession` interface (`apps/agent/src/voice/realtimeClient.ts`) that is MOCKED in tests** — the live round-trip + cross-platform device matrix are deferred (need real keys/devices; `docs/voice-cross-platform-smoke.md` is the script). **F-11 consumes this:** it reads the cache-friendly persona (`voice/persona.ts`) and the transcript stream off the same `RealtimeSession` seam — extend that interface, don't reshape it; and note the live transcript path is unverified until a human runs the smoke with keys. **MR !5 review hardened the seam:** `RealtimeSession.connect(config)` now *takes* the `RealtimeSessionConfig` (the bridge builds the persona prompt + cache key from lesson state and passes it at connect time) rather than the session being fixed at construction — F-11 builds on the updated signature. Two voice-wiring lessons the bot caught that the next voice feature should heed: **(1) joining a room ≠ being heard** — you must `publishTrack` the local mic after `room.connect` (F-11's explain-back captures learner audio too); **(2) a partially-configured external integration must fail closed** — a set key/secret but empty `LIVEKIT_URL` now returns 503, not a 201 with `url:""`. And client-side, never feed an unvalidated JSON numeric (`Number(body.expiresAt)`) into a timer delay without a `Number.isFinite` guard — NaN delays become a 0ms tight loop. |
...
- Production-grade auth + multi-tenant data isolation. **Voice raises the stakes here (F-10):** sessions are anonymous
  capabilities (the `sessionId` is the de-facto bearer token across the whole WS protocol, with no `learnerId`/owner on
  the `sessions` row), so a leaked `sessionId` now also grants join to that session's *live audio* room — an
  eavesdrop/inject escalation over the prior text-only surface (FERPA-relevant per ADR-004). F-10 does not weaken this;
  it inherits it. When auth lands, add a `learnerId` to `sessions` and verify session ownership in
  `POST /api/realtime/session` before minting. Until then: `sessionId`s only over TLS, never logged. (The mint endpoint
  is rate-limited per session to cap the cost-amplification side of the same surface.)
