...
Built test-first to the approved checklist. All items DONE except those the plan
defers (the ~30 real explain-back recordings; the live cross-platform device smoke).

**Gate + counter + config (T-12a/d-config/e):** `evaluateMasteryGate(state, config):
{passed, blockers}` added as a SIBLING in `gate.ts`; `isMastered` is now a one-line
delegate (boolean signature preserved for `server.ts` + `gate.test.ts`). Rule-gate
sub-blockers fold under `'rule_gate_failed'`. Topic-guardrail counts the AGENT's
off-topic `answer_question` Actions through the REAL `deriveState` fold (`offTopicCount`,
`topicGuardrailClean = offTopicCount <= budget`) — the hardcoded `topicGuardrailClean:
true` at eventConsumer.ts is KILLED. `MasteryConfig` gains OPTIONAL `topicGuardrailBudget`
(`.default(3)`) + `explainBackJudgeAgreementThreshold` (`.optional()`); a pre-F-12 config
still validates (round-trip tested) so the agent boots.

**Explain-back wire + the F-11 seam:** `toLoggedEvent` reads `payload.explainBackVerdict
.passed`; `deriveState` latches `explainBackPassed` on a folded passing verdict;
`toLearnerState` threads it (replacing the hardcoded `false`). Fail-closed: no verdict →
false → `explain_back_not_passed` blocker → block. F-11 owns the production judge/route;
since F-11 has not landed in this worktree, the verdict is produced by a DEV-ONLY seam
(`?testExplainBackVerdict=pass|fail`, `NODE_ENV!=='production'`) and persisted at the
shared `payload.explainBackVerdict` slot (mirroring `transferVerdict`). The read/fold/
persist sites are exactly what F-11's route re-points at — the seam is removed on the
serial join, the wiring is unchanged. The persist-slot line is the additive, identical-
shape edit the convergence table assigns to F-11; landed here so the path is drivable.

**Earned-it gate + named blockers (T-12-earned-it, AC#3):** `rejectUnauthorizedAction`'s
`transition→mastered` branch switched from `isMastered` to `evaluateMasteryGate`; on
rejection it returns `mastery_gate_failed: <blockers>`. The single per-turn
`evaluateMasteryGate` call feeds BOTH the earned-it rejection AND the persisted
`statechartDecision`/`statechartReason` + per-turn `gateEvaluation` (AC#5 replay). The
server (not the un-driven XState machine) is the truth-maker, per the verified finding.

**Reflex + renderer + seams:** an ACCEPTED mastery transition reflexively resolves to a
`MasteryCelebration` mount listing the KCs the learner actually mastered (BKT ≥ threshold,
server-sourced — not the agent's claim). `MasteryCelebration.tsx` renders concepts + a
disabled "Continue to Lesson 2" affordance (enabled only with `nextLessonId`, wired in
F-15); split out of the Tbd group in `registry.tsx` (the `never` default kept). The
prompt + stub now gate the organic proposal on the FULL gate (not the rule gate alone).
`?testForce=mastered` (dev-only) forces a transition proposal that the earned-it gate
still rejects — the demoable mastery-without-conditions refusal.

**Verification:** `pnpm typecheck` green (registry exhaustiveness, snapshot threading,
no one-sided menu edit). `pnpm test` 485 passed / 1 skipped / 0 failed. The REACHABILITY
integration test drives a RAW ClientEvent sequence through `handleClientFrame` (real fold)
to mastery: agent organically proposes → earned-it gate accepts → MasteryCelebration
mounts → replay shows `gateEvaluation` flipping failing→passing. Negative test: off-topic
budget tripped via REAL `learner_question` turns → mastery blocked with
`topic_guardrail_exceeded`. AC#3 test: `?testForce=mastered` → `no_action` +
`statechartReason: mastery_gate_failed: ...`. `docker build` of `apps/agent` succeeds; the
image BOOTS in `NODE_ENV=production` (migrations + seed run), serves `/api/health` 200, and
the dev seam is verified INERT in prod (a forced-mastery query param does nothing).
Deferred per the plan: the ~30 real recordings + the live device smoke.
