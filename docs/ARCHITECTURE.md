...
5. **Whether the playground (L5 capstone) lives in its own substate or its own micro-statechart.** Free-build mode is structurally different from directed practice; [ADR-002](./adrs/ADR-002-curriculum-scope-and-mvp-cut.md) acknowledges this and defers the choice to implementation time.
